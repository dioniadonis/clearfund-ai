...
const OperatorLeads: React.FC = () => {
  ...
  const query = useQuery({
    queryKey: ["operator", "leads", search, stage, page],
    queryFn: async () => {
      let q = supabase
        .from("leads")
        .select("id, full_name, business_name, email, stage, next_action, next_action_due", {
          count: "exact",
        })
  ...
  return (
    <div className="space-y-6">
      ...
      <LeadDetailSheet leadId={selected} onOpenChange={(open) => !open && setSelected(null)} />
    </div>
  );
};
